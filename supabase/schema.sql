...
create policy profiles_insert_first_admin on public.profiles
for insert to authenticated
with check (
  id = auth.uid()
  and role = 'admin'
  and not exists (select 1 from public.profiles where role = 'admin' and active = true)
);
...
for select to authenticated
using (profile_id = auth.uid() or app_private.is_staff());

drop policy if exists student_access_maps_staff_write on public.student_access_maps;
create policy student_access_maps_staff_write on public.student_access_maps
for all to authenticated
using (app_private.is_staff())
...
create policy homework_staff_write on public.homework
for all to authenticated
using (app_private.can_access_classroom(classroom_id))
with check (app_private.can_access_classroom(classroom_id));
...
create policy homework_status_staff_write on public.homework_status
for all to authenticated
using (
  exists (
...
create policy behavior_staff_write on public.behavior
for all to authenticated
using (app_private.can_access_classroom(classroom_id))
with check (app_private.can_access_classroom(classroom_id));
...
create policy scores_staff_write on public.scores
for all to authenticated
using (app_private.can_access_classroom(classroom_id))
with check (app_private.can_access_classroom(classroom_id));
...
create policy sync_runs_staff_all on public.sync_runs
for all to authenticated
using (app_private.is_staff())
with check (app_private.is_staff());
